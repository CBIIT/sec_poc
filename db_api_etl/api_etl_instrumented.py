...
times.append(['get maintypes','-' ,(t2-t1).total_seconds()])
...
r = requests.post('https://clinicaltrialsapi.cancer.gov/v1/clinical-trials',
                  data=data)
t2=datetime.datetime.now()
times.append(['get trials - total number','-' ,(t2-t1).total_seconds()])
t = r.text
j = r.json()
# print(j)
total = j['total']
...
while run:
    ...
    r = requests.post('https://clinicaltrialsapi.cancer.gov/v1/clinical-trials',
                      data=data)
    t2 = datetime.datetime.now()
    times.append(['get trials - start at:', start,(t2 - t1).total_seconds()])
...
with open('v1_etl_'+datetime.datetime.now().strftime("%Y_%m_%d_%I_%M_%S_%p")+'.csv', 'w', newline='') as f:
    writer = csv.writer(f)
